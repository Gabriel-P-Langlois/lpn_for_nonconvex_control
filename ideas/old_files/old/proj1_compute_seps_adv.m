...
m = 50000; R = size^2;   % m = number of samples, R = subsampling rate ( = number of pixels).
...
for k=alpha_ind
    ...
    for i=1:1:m
        ...
        temp_ind_2 = randi(size,[R,2]);               % Pick two indices
    ...
    u_pm_1 = (1/m)*(u_pm_1);
    u_pm_2 = (1/m)*(u_pm_2);
    norm_diff1 = norm(u_pm_1-u_pm_2,'fro');
    norm_diff2 = max(max(abs(u_pm_1-u_pm_2)));
