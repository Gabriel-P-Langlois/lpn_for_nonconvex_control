...
step_size = (0.0001/(n^(1/3)))*(t*epsilon);
...
for k=burn_in:1:(burn_in+iter-1)
    ...
    grad_prior_emp = grad_prior_emp + grad_prior(y1,x,t,lambda,mu)/iter;
end
toc
u_empirical = x-sqrt(t)*u_empirical;
%% ERROR ANALYSIS
...
function val = F(y,x,t,epsilon,lambda,mu)
    val = -(0.5*norm(y)^2 + prior(y,x,t,lambda,mu))/epsilon; 
    
    function prior_val = prior(y,x,t,lambda,mu)
        y = x-sqrt(t)*y;
...
function vector = gradF(y,x,t,epsilon,lambda,mu)
    vector = -(y + grad_prior(y,x,t,lambda,mu))/epsilon;
    
    function prior_grad_vector = grad_prior(y,x,t,lambda,mu)
        y = x-sqrt(t)*y;
