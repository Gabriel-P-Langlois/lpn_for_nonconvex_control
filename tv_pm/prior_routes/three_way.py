...
METHODS
  ...
  u_t (LPN Iterative recovery)     grad psi_theta(x)              -- one forward pass
  ...
  u_t (One-shot recovery)     solve grad G_theta(u) = x      -- convex solve on the conjugate net
                      prox_{J_2}(x) = argmin_u G(u) - <u,x>,  J_2 = G - 0.5||.||^2
  u_hat (direct)    prox_{J_theta}(x) by L-BFGS    -- the existing notebook method

Both recoveries rest on ONE trained denoiser, psi_theta, fitted by gradient
...
# CHECKPOINT tag. Set TW_SUFFIX to keep a retrain off the shipped checkpoints.
SUF = os.environ.get("TW_SUFFIX", "")
# OUTPUT tag, deliberately SEPARATE from SUF. A retrain that silently overwrote
# results/figs/threeway_cameraman.png with a 3000-step smoke net would replace a
# reported figure with a throwaway one and leave no trace. Set TW_OUT_SUFFIX and
# the figures and tables land beside the originals instead of on top of them.
OUT_SUF = os.environ.get("TW_OUT_SUFFIX", "")
...
def conjugate_pairs_x(psi, u, x):
    ...
    convex and  argmin_u G(u) - <u,x>  (the one-shot recovery's denoiser) is a well-posed
    ...
    modelling choice, but they decide whether the one-shot recovery's own program is coercive.
    """
...
def train_G(steps, seed=1):
    ...
    Value regression, exactly as bin/_run.py does for One-shot recovery; the pairs are
...
def train_G_grad(steps, seed=1):
    ...
    WHY THIS VARIANT EXISTS. bin/_run.py's One-shot recovery regresses psi*'s VALUES, and
    the synthetic notebooks score One-shot recovery on prior VALUES, so a value fit is all
    that protocol ever needs. the one-shot recovery's DENOISER, however, is the inversion of G,
    ...
    to four digits. This variant hands One-shot recovery exactly the supervision its own
...
def denoise_route1(psi, u, x):
    """u_t(LPN Iterative recovery) = grad psi_theta(x). Exact prox of the iterative recovery's recovered prior."""
    return u.grad(psi, flat(x)).reshape(x.shape)


def denoise_route2(G, u, x, iters=1500, tol=1e-12):
    """u_t(One-shot recovery) = argmin_u G_theta(u) - <u, x>, i.e. grad G(u_hat) = x.

    J_2 = G - 0.5||.||^2 is the one-shot recovery's recovered prior, so
    ...
    # the iterative recovery's inversion with Adam rather than a line-searched method, so using
    # Adam here keeps the two recoveries on the same footing.
