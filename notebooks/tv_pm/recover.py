"""Steps 2-3: fit one convex net to the denoiser's gradients, and score it.
    ...
    ~/miniforge3/envs/lpn_env/bin/python notebooks/tv_pm/recover.py            # ~10 min
    ~/miniforge3/envs/lpn_env/bin/python notebooks/tv_pm/recover.py --sweeps 2000
...
def estimate_delta(x, sweeps, reps=8, n_x=16, seed=77):
    ...
    u = sample_pm(xb, dataset.SIGMA, dataset.LAM, sweeps=sweeps, w=1.0, seed=seed)["u_pm"]
...
def run(sweeps=dataset.SWEEPS, steps=STEPS, width=WIDTH, seed=1, save=True,
    ...
    print(f"  trained in {(time.time()-t0)/60:.1f} min; best val loss {hist['best_val']:.4e}")

    delta = estimate_delta(ev["x"], sweeps)
    ...
    if save:
        ...
        ck = os.path.join(ROOT, "logs", "ckpt",
                          f"tv_pm_{n}D_{arch}_m{sweeps}_{tag}_b{beta}_s{steps}.pth")
        os.makedirs(os.path.dirname(ck), exist_ok=True)
        torch.save({"state": model.state_dict(), "arch": arch, "in_dim": n,
                    "hidden": width, "layers": layers, "channels": channels,
                    "blocks": blocks, "beta": beta, "sweeps": sweeps, "mu": u.mu,
                    "s": u.s, "sigma": dataset.SIGMA, "lam": dataset.LAM}, ck)
        print(f"  -> {ck}")
...
def main():
    ...
    run(sweeps=a.sweeps, steps=a.steps, width=a.width, save=not a.no_save,
        ...
        arch=a.arch, channels=a.channels, blocks=a.blocks)
