"""The prior IN ACTION: denoise with the learned f_reg, compare to the denoiser.
    ...
    ~/miniforge3/envs/lpn_env/bin/python notebooks/tv_pm/denoise_demo.py   (~2 min)
...
MODELS = {"fc": _find("tv_pm_64D_m8000_w256_b20.0_s250000.pth"),
          "conv": _find("tv_pm_64D_conv_m8000_C32_B2_b20.0_s250000.pth")}
...
def run(image="cameraman_256x256_d", arch="fc", sigma=dataset.SIGMA,
        lam=dataset.LAM, sweeps=8000, seed=3):
    clean = dataset.load_image if False else None            # (silence linters)
    from scipy.io import loadmat
    img = np.asarray(loadmat(os.path.join(ROOT, "ideas", "old_files",
                                          image + ".mat"))[image], dtype=float)
    H, W = img.shape
    xc = tile(img)                                           # clean patches
    rng = np.random.default_rng(seed)
    xn = np.clip(xc + rng.normal(0, sigma, xc.shape), 0, 1)  # noisy patches x_k

    print(f"  image {image} ({H}x{W}), {xn.shape[0]} patches of 8x8, sigma={sigma:.4f}")
    ...
    fig.suptitle(f"Denoising with the RECOVERED prior vs the true posterior-mean "
                 ...
                 f"SSIM {s_match:.4f}, rel-L2 {100*r_match:.1f}%", fontsize=12)
    fig.tight_layout(rect=[0, 0, 1, 0.94])
    os.makedirs(OUT, exist_ok=True)
    for ext in ("png", "pdf"):
        fig.savefig(os.path.join(OUT, f"denoise_{arch}_{image.split('_')[0]}.{ext}"),
                    dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"  -> {OUT}/denoise_{arch}_{image.split('_')[0]}.png/pdf")
...
def main():
    ...
    a = ap.parse_args()
    run(image=a.image, arch=a.arch, sweeps=a.sweeps)
